...
{
    public:
        std::string CLogin(std::string cport, std::string cip, int id);
        void set_port(std::string mport);
...
int main(int argc, char** argv) {
    ...
    std::string id;
    int opt = 0;
    while ((opt = getopt(argc, argv, "h:u:c:i:")) != -1){
        switch(opt) {
            /***case 'h':
    ...
    myc.set_port(myc.CLogin(cport, cip, id_no));
...
void Client::set_port(std::string mport){
    this->port = mport;
}
...
std::string Client::CLogin(std::string cport, std::string cip, int id){
    std::cout << "Trying to login" << std::endl;
    std::string login_info = cip + ":" + cport;

    //Creating the coordinator request
    CRequest req;
    req.set_port_number(port);
    req.set_server_type("CLIENT");
    req.set_id(id);
    req.set_requester("CLIENT");
    CReply rep;

    //Connecting to the coordinator
    ClientContext context;
    std::unique_ptr<SNSCoordinator::Stub> cstub_;
    cstub_ = std::unique_ptr<SNSCoordinator::Stub>(SNSCoordinator::NewStub(
               grpc::CreateChannel(
                    login_info, grpc::InsecureChannelCredentials())));
    std::cout << "Sending to Coordinator" << std::endl;
    Status status = cstub_->CLogin(&context, req, &rep);
    std::string sport = rep.msg(); 
    return sport;
}
